...
def compute_histograms_and_test():
    ...
    if not RECOMPUTE_HISTOGRAMS and os.path.exists(path.join(TEMPS_DIR,f'test_labels_{DICTIONARY_SIZE}.tiff')):
        ...
        train_labels = cv2.imread(path.join(TEMPS_DIR,f'train_labels_{DICTIONARY_SIZE}.tiff'), cv2.IMREAD_UNCHANGED).ravel()

        test_histograms = cv2.imread(path.join(TEMPS_DIR,f'test_histograms_{DICTIONARY_SIZE}.tiff'), cv2.IMREAD_UNCHANGED)
        test_labels = cv2.imread(path.join(TEMPS_DIR,f'test_labels_{DICTIONARY_SIZE}.tiff'), cv2.IMREAD_UNCHANGED).ravel()
    else:
        print('Computing histograms as descriptors for images')
    ...
    results = classifier.predict(test_histograms)
    ...
    if not FIND_BEST_HYPER_PARAMETERS:
        confusion_matrix = sklearn.metrics.confusion_matrix(test_labels, results)
        # cv2.imwrite(path.join(BASE_DIR,'outputs','images','res09.jpg'), confusion_matrix)
        pyplot.imshow(confusion_matrix)
        pyplot.savefig(path.join(BASE_DIR,'outputs','images','res09.jpg'))
        # pyplot.show()
...
if FIND_BEST_HYPER_PARAMETERS:
    for DICTIONARY_SIZE in range(50,201,20):
        print('Running tests with parameters (with SVM as query algorithm): ')
